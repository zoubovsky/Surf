...
export function evaluateRisk(input: RiskInput): RiskDecision {
  ...
  if (plan.action !== "enter") {
    ...
    return {
      ...
      checks,
      order: null,
      terminal: plan.action === "hold" ? "hold" : "no-op",
      summary: "not an entry",
    };
  }
  ...
  checks.push(
    check("not-halted", !state.halted, state.halted ? `halted: ${state.haltReason ?? "unknown"}` : "ok"),
  );
  checks.push(
    check(
      "llm-budget",
      state.llmSpendTodayUsd <= limits.dailyLlmBudgetUsd,
      `spend ${state.llmSpendTodayUsd.toFixed(2)} / ${limits.dailyLlmBudgetUsd}`,
    ),
  );
  ...
  checks.push(
    check(
      "prior-agrees",
      !plan.priorDisagrees,
      plan.priorDisagrees ? "analyst prior disagrees with count" : "ok",
    ),
  );
  ...
  checks.push(
    check("plan-complete", complete, complete ? "ok" : "missing direction/entry/stop/target/candidate"),
  );
  ...
  checks.push(
    check(
      "candidate-known",
      candidate !== null && candidate.id === plan.candidateId,
      candidate ? "ok" : "candidate not found",
    ),
  );
  if (candidate) {
    checks.push(
      check(
        ...
        `candidate=${candidate.direction} plan=${direction}`,
      ),
    );
    const beyond =
      direction === "long" ? stop <= candidate.invalidation.price : stop >= candidate.invalidation.price;
    checks.push(
      check(
        "stop-beyond-invalidation",
        beyond,
        `stop ${stop} vs invalidation ${candidate.invalidation.price}`,
      ),
    );
  }
  ...
  checks.push(
    check(
      "market-fresh",
      marketAge >= 0 && marketAge < 120_000,
      `market snapshot age ${Math.round(marketAge / 1000)}s`,
    ),
  );
  ...
  checks.push(
    check(
      "candle-fresh",
      candleAge <= limits.maxCandleAgeMs,
      `last candle age ${Math.round(candleAge / 60000)}m`,
    ),
  );
  if (market.referencePrice !== null) {
    ...
    checks.push(
      check(
        "reference-deviation",
        dev <= limits.maxReferenceDeviationPct,
        `mark vs reference ${dev.toFixed(2)}%`,
      ),
    );
  } else {
  ...
  checks.push(
    check(
      "account-fresh",
      accountAge >= 0 && accountAge < 120_000,
      `account snapshot age ${Math.round(accountAge / 1000)}s`,
    ),
  );
  ...
  checks.push(
    check(
      "max-positions",
      account.openPositions.length < limits.maxConcurrentPositions,
      `open=${account.openPositions.length} max=${limits.maxConcurrentPositions}`,
    ),
  );
  const dailyLossPct =
    state.dayStartEquity > 0 ? ((state.dayStartEquity - account.equity) / state.dayStartEquity) * 100 : 0;
  checks.push(
    check(
      "daily-loss",
      dailyLossPct < limits.maxDailyLossPct,
      `daily loss ${dailyLossPct.toFixed(2)}% max ${limits.maxDailyLossPct}%`,
    ),
  );
  const ddPct =
    state.highWaterEquity > 0 ? ((state.highWaterEquity - account.equity) / state.highWaterEquity) * 100 : 0;
  checks.push(
    check(
      "drawdown",
      ddPct < limits.maxDrawdownPct,
      `drawdown ${ddPct.toFixed(2)}% max ${limits.maxDrawdownPct}%`,
    ),
  );
  checks.push(
    check(
      "entries-today",
      state.entriesToday < limits.maxEntriesPerDay,
      `entries today ${state.entriesToday} max ${limits.maxEntriesPerDay}`,
    ),
  );
  ...
  checks.push(
    check(
      "entry-spacing",
      sinceLast >= limits.minHoursBetweenEntries,
      `hours since last entry ${Number.isFinite(sinceLast) ? sinceLast.toFixed(1) : "n/a"}`,
    ),
  );
  checks.push(
    check(
      "consecutive-stop-outs",
      state.consecutiveStopOuts < limits.maxConsecutiveStopOuts,
      `consecutive stop-outs ${state.consecutiveStopOuts}`,
    ),
  );
  ...
  if (plan.entryKind === "limit") {
    ...
    checks.push(
      check(
        "resting-entry-distance",
        distPct <= 4.5,
        `resting entry ${distPct.toFixed(2)}% from mark (venue bound 5%)`,
      ),
    );
  }
  ...
  checks.push(
    check(
      "funding-not-extreme",
      adverse <= limits.maxAdverseFundingHourly,
      `adverse funding ${(adverse * 100).toFixed(4)}%/h`,
    ),
  );
  ...
  checks.push(
    check(
      "sizing",
      true,
      `size ${sized.size} notional ${sized.notionalUsd.toFixed(2)} lev ${sized.leverage}`,
    ),
  );
  checks.push(
    check(
      "leverage-cap",
      sized.leverage <= limits.maxLeverage,
      `leverage ${sized.leverage} max ${limits.maxLeverage}`,
    ),
  );
  if (market.depthNotionalNear !== null) {
    checks.push(
      check(
        "depth",
        sized.notionalUsd <= market.depthNotionalNear * limits.maxDepthFraction,
        `notional ${sized.notionalUsd.toFixed(0)} vs depth ${market.depthNotionalNear.toFixed(0)}`,
      ),
    );
  } else {
  ...
  checks.push(
    check(
      "reward-risk",
      rr >= limits.minRewardRisk,
      `R:R after costs ${rr.toFixed(2)} min ${limits.minRewardRisk}`,
    ),
  );
