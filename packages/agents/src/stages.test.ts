...
describe("triage", () => {
  it("uses the triage model with no thinking, no effort and a small max_tokens", async () => {
    const client = createFakeClient({ onParse: () => ({ relevant: true, isBitcoinAnalysis: true, substantive: true, reason: "levels and count" }) });
  ...
  it("surfaces refusals and truncation as typed errors", async () => {
    const refusing = createFakeClient({ onParse: () => fakeResponse({ stop_reason: "refusal", stop_details: { type: "refusal", category: "general_harms", explanation: "x" } }) });
    await expect(triage({ client: refusing, model: "claude-haiku-4-5" }, "t", "t")).rejects.toBeInstanceOf(LlmRefusalError);
    const truncating = createFakeClient({ onParse: () => fakeResponse({ stop_reason: "max_tokens" }) });
    await expect(triage({ client: truncating, model: "claude-haiku-4-5" }, "t", "t")).rejects.toBeInstanceOf(LlmTruncatedError);
  });
});
...
describe("extractPrior", () => {
  it("runs Opus with adaptive thinking at high effort, overrides identity fields and verifies evidence", async () => {
    const modelOutput = { ...F.prior(), videoId: "WRONG", asset: "BTC", targets: [...F.prior().targets, { price: 99_000, label: "invented" }] };
    const client = createFakeClient({ onParse: () => modelOutput });
    const r = await extractPrior({ client, model: "claude-opus-5" }, { videoId: "vid1", title: "T", publishedAt: F.NOW - 6 * F.H, transcriptText: F.TRANSCRIPT, keywordWindows: [] });
    const p = client.parseCalls[0]!.params;
  ...
  it("rejects an output that violates the schema even after clipping", async () => {
    ...
    await expect(
      extractPrior({ client, model: "claude-opus-5" }, { videoId: "vid1", title: "T", publishedAt: F.NOW, transcriptText: F.TRANSCRIPT, keywordWindows: [] }),
    ).rejects.toThrow();
  });
});
...
describe("research", () => {
  ...
  it("configures the runner with the allow-listed web search and the numbers tool, resumes pause_turn, then coerces", async () => {
    ...
    const client = createFakeClient({
      onToolRunner: () => [paused, fakeMessage("Regime: trending-up\nFunding: neutral\nBrief: BTC up.", { stop_reason: "end_turn" })],
    ...
    const web = rp.tools.find((t) => (t as { type?: string }).type === "web_search_20260209") as { max_uses: number; allowed_domains: string[]; name: string };
    expect(web.name).toBe("web_search");
    expect(web.max_uses).toBe(4);
    expect(web.allowed_domains).toEqual(["coindesk.com", "theblock.co", "cointelegraph.com", "reuters.com", "bloomberg.com", "cmegroup.com", "federalreserve.gov", "forexfactory.com"]);
    const numbers = runnable(rp, "get_market_numbers")!;
    expect(JSON.parse((await numbers.run({}, { toolUse: { type: "tool_use", id: "x", name: "get_market_numbers", input: {} }, toolUseBlock: { type: "tool_use", id: "x", name: "get_market_numbers", input: {} } })) as string)).toMatchObject({ markPrice: 78_400 });
    // pause_turn handling
  ...
  it("fails loudly when the runner ends without text", async () => {
    const client = createFakeClient({
      onToolRunner: () => [{ content: [], stop_reason: "pause_turn", stop_details: null, usage: { input_tokens: 1, output_tokens: 1 }, model: "m" }],
      onParse: () => F.context(),
    });
    await expect(research({ client, model: "claude-sonnet-5" }, input)).rejects.toBeInstanceOf(LlmOutputError);
  });

  it("aborts on a refusal mid-run", async () => {
    const client = createFakeClient({ onToolRunner: () => [fakeMessage("", { stop_reason: "refusal" })], onParse: () => F.context() });
    await expect(research({ client, model: "claude-sonnet-5" }, input)).rejects.toBeInstanceOf(LlmRefusalError);
  });
});
...
describe("analyze", () => {
  ...
  it("routes reviewer feedback through a system message on Opus 5 and a user block elsewhere", () => {
    const revision = { round: 1, review: F.verdict({ verdict: "revise", reasons: ["widen the stop buffer"] }) };
...
describe("review", () => {
  ...
  const tools = () => createReviewerTools({ ew: input.ew, prior: input.prior, market: input.market, limits: input.limits });
  ...
  it("exposes the four deterministic tools, uses the adversarial prompt and coerces the verdict", async () => {
    const client = createFakeClient({
      ...
      onParse: () => F.verdict({ adjustedConfidence: "medium", checks: { ...F.verdict().checks, rewardRiskRecomputed: null } }),
    });
    const r = await review({ client, model: "claude-opus-5" }, input, tools());
    const rp = client.runnerCalls[0]!.params;
    expect(stageOf(rp)).toBe("review");
    expect(rp.tools.map((t) => (t as { name: string }).name).sort()).toEqual(["check_stop_vs_invalidation", "get_candidate", "get_prior_levels", "recompute_reward_risk"]);
    expect(rp.thinking).toEqual({ type: "adaptive" });
    ...
    const ctx = { toolUse: { type: "tool_use" as const, id: "1", name: "x", input: {} }, toolUseBlock: { type: "tool_use" as const, id: "1", name: "x", input: {} } };
    expect(JSON.parse((await runnable(rp, "get_candidate")!.run({ id: "1h:imp-3" }, ctx)) as string)).toMatchObject({ id: "1h:imp-3" });
    expect(JSON.parse((await runnable(rp, "recompute_reward_risk")!.run({}, ctx)) as string).rewardRisk).toBeCloseTo(1.697, 2);
    expect(JSON.parse((await runnable(rp, "check_stop_vs_invalidation")!.run({}, ctx)) as string).ok).toBe(true);
  ...
  it("clamps adjustedConfidence to the plan's confidence", async () => {
    const client = createFakeClient({ onToolRunner: () => [fakeMessage("findings")], onParse: () => F.verdict({ adjustedConfidence: "high" }) });
    const r = await review({ client, model: "claude-opus-5" }, { ...input, plan: F.plan({ confidence: "medium" }) }, tools());
    expect(r.output.adjustedConfidence).toBe("medium");
  ...
  it("downgrades an approve to revise when evidence is untraceable", async () => {
    ...
    const client = createFakeClient({ onToolRunner: () => [fakeMessage("findings")], onParse: () => F.verdict() });
  ...
  it("forces reject when the candidate does not exist", () => {
    const pre = { unknownEvidence: [], candidateExists: false, marketAgeSec: 0, accountAgeSec: 0, candleAgeMin: 1, priorFreshness: null };
  ...
  it("recognises headline/event indices and indicator names as traceable", () => {
    const plan = F.plan({ evidence: ["headline:0", "event 0", "RSI14 = 42", "funding neutral", "headline:7"] });
...
describe("post-trade review, daily brief, answer", () => {
  it("postTradeReview runs Opus at medium effort and anchors the lesson to the trade", async () => {
    const client = createFakeClient({
      onParse: () => ({ decisionQuality: "acceptable", outcome: "loss", failureMode: "stop-too-tight", lesson: { text: "widen buffers", evidenceTradeIds: ["t0"] }, summary: "s" }),
    });
  ...
  it("answerQuestion has no tools and wraps the question", async () => {
    ...
    const r = await answerQuestion(
      ...
      { question: "positions?", context: { asOf: F.NOW, positions: [], pnl: { todayUsd: 0 }, recentDecisions: [], limits: F.limits() } },
    );
