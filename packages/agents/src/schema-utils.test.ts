...
describe("schema-utils", () => {
  it("lenient strips string/array length checks but keeps shape", () => {
    const strict = z.object({
      a: z.string().max(3),
      b: z.array(z.string().max(2)).max(1),
      c: z.number().positive().nullable(),
    });
  ...
  it("conform clips strings and arrays to the strict maxima", () => {
    const strict = z.object({
      a: z.string().max(5),
      b: z.array(z.string().max(2)).max(1),
      n: z.object({ s: z.string().max(3) }).nullable(),
    });
    expect(conform(strict, { a: "abcdefgh", b: ["xyz", "q"], n: { s: "abcd" } })).toEqual({
      a: "abcd…",
      b: ["x…"],
      n: { s: "ab…" },
    });
  ...
  it("finalize yields the exact core type for an over-long brief", () => {
    const ctx = finalize(MarketContext, {
      ...F.context(),
      brief: "x".repeat(2_000),
      headlines: Array(12).fill("h"),
    });
