...
import { assertVideoId, buildTranscript, type Transcript, type TranscriptProvider, type TranscriptSegment } from "./types.js";
...
const ErrorBody = z.object({ error: z.string().optional(), message: z.string().optional(), details: z.string().optional() }).passthrough();
...
export class SupadataProvider implements TranscriptProvider {
  ...
  async fetch(videoId: string, lang = "en"): Promise<Transcript | null> {
    ...
    const url = new URL(this.endpoint === "youtube" ? `${this.baseUrl}/youtube/transcript` : `${this.baseUrl}/transcript`);
    ...
    if (res.status === 202) {
      ...
      if (!accepted.success) throw new TranscriptError("supadata: 202 without jobId", { provider: this.name, retryable: true, status: 202 });
    ...
    if (!body.success) throw new TranscriptError("supadata: unexpected 200 body", { provider: this.name, retryable: true, status: 200 });
  ...
  private async pollJob(jobId: string, videoId: string): Promise<Transcript | null> {
    ...
    for (;;) {
      ...
      if (!job.success) throw new TranscriptError("supadata: unexpected job status body", { provider: this.name, retryable: true, status: 200 });
      if (job.data.status === "completed") {
        if (job.data.content === undefined) return null;
        return this.toTranscript(videoId, job.data.lang ?? "en", { content: job.data.content, ...(job.data.lang ? { lang: job.data.lang } : {}) });
      }
      if (job.data.status === "failed") {
        throw new TranscriptError(`supadata: job failed: ${JSON.stringify(job.data.error ?? null).slice(0, 200)}`, {
          provider: this.name,
          retryable: true,
        });
      }
      if (this.clock.now() - started > this.maxPollMs) {
        throw new TranscriptError(`supadata: job ${jobId} still ${job.data.status} after ${this.maxPollMs}ms`, {
          provider: this.name,
          retryable: true,
        });
      }
  ...
  private async request(url: string): Promise<Response> {
    ...
    let res: Response;
    try {
      res = await this.http(url, { method: "GET", headers: { "x-api-key": this.opts.apiKey, accept: "application/json" } });
    } catch (err) {
      throw new TranscriptError(`supadata: network error: ${(err as Error).message}`, { provider: this.name, retryable: true, cause: err });
    }
  ...
  private async toError(res: Response): Promise<TranscriptError> {
    ...
    switch (res.status) {
      ...
      default:
        return new TranscriptError(`supadata: ${detail}`, { provider: this.name, retryable: res.status >= 500, status: res.status });
    }
  }
  ...
  private toTranscript(videoId: string, requestedLang: string, body: TranscriptBody): Transcript | null {
    ...
    if (typeof body.content === "string") {
      ...
      segments = body.content.map((c) => ({ start: c.offset / 1000, duration: (c.duration ?? 0) / 1000, text: c.text }));
    }
