...
- Deployed by a GitHub Actions workflow that runs _on that runner_: it receives repository secrets from GitHub, writes `/etc/surf/surf.env` (mode 0600), and runs `docker compose up -d`.
